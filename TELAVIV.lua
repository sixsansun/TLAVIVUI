...
-- ═══════════════════════════════════════════════════════════════
-- TEL-AVIV NOTIFICATION SYSTEM
-- Top-right toast stack styled after the main TEL-AVIV panel.
--
-- Compatible calls:
--   Library:Notify("Saved")
--   Library:Notify("Config — Saved : TESTCONFIG", 3)
--   Library:Notify("Config", "Saved : TESTCONFIG", 3)
--   Library:Notify({
--       Title = "Config",
--       Content = "Saved : TESTCONFIG",
--       Duration = 3,
--       Color = Color3.fromRGB(65, 139, 235)
--   })
-- ═══════════════════════════════════════════════════════════════

Library._NotificationState = Library._NotificationState or {
    Gui = nil,
    Container = nil,
    Active = {},
    Counter = 0,
...
local function NotificationColorFromType(kind)
    kind = string.lower(tostring(kind or "info"))

    if kind == "success" then
        return Color3.fromRGB(74, 190, 126)
    elseif kind == "warning" or kind == "warn" then
        return Color3.fromRGB(232, 178, 72)
    elseif kind == "error" or kind == "danger" then
        return Color3.fromRGB(226, 83, 91)
    end

    return COLORS.BlueBright
end

local function NormalizeNotificationArguments(a, b, c, d)
    local data = {
        Title = "TEL-AVIV",
        Content = "",
        Duration = 3,
    ...
    if type(a) == "table" then
        ...
        data.Duration = tonumber(a.Duration or a.Time or 3) or 3
        data.Type = tostring(a.Type or "info")
        data.Color = typeof(a.Color) == "Color3"
            and a.Color
            or NotificationColorFromType(data.Type)
        return data
    end

    -- Library:Notify("Title", "Content", 3)
    if type(b) == "string" then
        data.Title = tostring(a or "TEL-AVIV")
        data.Content = tostring(b or "")
    ...
    -- Compatibility with the existing Panel Wars wrapper:
...
local function EnsureNotificationContainer()
    local state = Library._NotificationState

    if state.Gui and state.Gui.Parent and state.Container and state.Container.Parent then
        return state
    end
...
local function RemoveActiveNotification(item)
    local state = Library._NotificationState
...
function Library:Notify(a, b, c, d)
    local data = NormalizeNotificationArguments(a, b, c, d)
    local state = EnsureNotificationContainer()
    if not state then
        return nil
    end
    ...
    local width = 326
    local height = (data.Content ~= "" and 82 or 65)
    local duration = math.max(0.65, data.Duration or 3)
    ...
    local cardStroke = Stroke(card, COLORS.Stroke, 1, 0.16)
    Shadow(
        card,
        Color3.new(0, 0, 0),
        0.58,
    ...
    -- Soft accent wash on the left; this is deliberately a gradient,
    -- not a bright rectangular outline.
    ...
    local accentLine = New("Frame", {
        ...
        Size = UDim2.fromOffset(3, height - 20),
        ZIndex = 105,
    }, card)
    Corner(accentLine, 2)
    ...
    title.Name = "Title"
    title.Position = UDim2.fromOffset(37, data.Content ~= "" and 9 or 0)
    title.Size = UDim2.new(1, -74, 0, data.Content ~= "" and 24 or height - 8)
    ...
    if data.Content ~= "" then
        ...
        contentLabel.Position = UDim2.fromOffset(37, 32)
        contentLabel.Size = UDim2.new(1, -56, 0, 32)
    ...
    -- Progress bar copies the blue/glow language used by the TEL-AVIV sliders.
    ...
    Corner(progressGlow, 4)

    New("UIGradient", {
        Color = ColorSequence.new(accent),
        Transparency = NumberSequence.new({
            NumberSequenceKeypoint.new(0, 1),
            NumberSequenceKeypoint.new(0.28, 0.74),
            NumberSequenceKeypoint.new(0.5, 0.12),
            NumberSequenceKeypoint.new(0.72, 0.74),
            NumberSequenceKeypoint.new(1, 1),
        }),
        Rotation = 90,
    }, progressGlow)
    ...
    New("UIGradient", {
        Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, accent:Lerp(Color3.new(0, 0, 0), 0.22)),
    ...
    close.MouseEnter:Connect(function()
        Tween(close, 0.10, {TextColor3 = COLORS.Text})
    end)

    close.MouseLeave:Connect(function()
        Tween(close, 0.10, {TextColor3 = COLORS.Muted2})
    end)

    close.MouseButton1Click:Connect(dismiss)

    -- Opening: the slot expands first while the card slides in from the right.
    Tween(wrapper, 0.18, {
        Size = UDim2.fromOffset(width, height),
    })
...
function Library:ClearNotifications()
    ...
    for _, item in ipairs(state.Active or {}) do
