...
Library.Version = "4.4.1-theme-logo-mask"
...
Library.Theme = Library.Theme or {
    ...
    AccentSelection = COLORS.BlueSelection,
}

local TA_AccentExtraColors = {
    ToggleOn = Color3.fromRGB(45, 111, 190),
    IconHolder = Color3.fromRGB(24, 55, 91),
    IconHolderSelected = Color3.fromRGB(26, 62, 105),
    SelectedText = Color3.fromRGB(181, 209, 248),
}

local function TA_ThemeSameColor(a, b)
...
local function TA_ThemeDerived(accent)
    local black = Color3.new(0, 0, 0)
    local white = Color3.new(1, 1, 1)

    return {
        Blue = accent,
        BlueBright = accent:Lerp(white, 0.22),
        BlueDark = accent:Lerp(black, 0.38),
        BlueSelection = accent:Lerp(black, 0.70),

        ToggleOn = accent:Lerp(black, 0.25),
        IconHolder = accent:Lerp(black, 0.72),
        IconHolderSelected = accent:Lerp(black, 0.62),
        SelectedText = accent:Lerp(white, 0.62),
    }
end

local function TA_ThemeMapColor(color, oldTheme, newTheme)
    if typeof(color) ~= "Color3" then
        return color
    end

    for key, oldColor in pairs(oldTheme) do
        if typeof(oldColor) == "Color3"
            and TA_ThemeSameColor(color, oldColor)
        then
            local replacement = newTheme[key]

            if typeof(replacement) == "Color3" then
                return replacement
            end
        end
    end
...
local function TA_ThemeMapSequence(sequence, oldTheme, newTheme)
...
local function TA_ThemeRecolorObject(object, oldTheme, newTheme)
    if not object then
        return
    end
    ...
    elseif object:IsA("UIShadow") then
        pcall(function()
            object.Color =
                TA_ThemeMapColor(
                    object.Color,
...
local function TA_ThemeRecolorTree(root, oldTheme, newTheme)
    if not root then
        return
    end

    TA_ThemeRecolorObject(
        root,
        oldTheme,
        newTheme
    )
...
local function TA_PrepareTintableLogo(window, assetUri)
    if not window
        or not window.Logo
        or not assetUri
    then
    ...
    if window._LogoMaskPreparing
        or window._LogoMaskReady
    then
        return
    end

    window._LogoMaskPreparing = true
        ...
        local ok, err =
            pcall(function()
                ...
                if not sourceImage then
                    error(
                        "CreateEditableImageAsync returned nil"
                    )
                end
                ...
                -- Convert the original colored logo into a neutral mask.
                --
                -- RGB becomes pure white.
                -- Original color intensity becomes alpha, so the logo keeps
                -- its antialiasing / highlights / transparent silhouette.
                --
                -- Once neutral, ImageColor3 can generate ANY theme hue.
                for pixel = 0, pixelCount - 1 do
                    ...
                    -- max(R,G,B) works much better than luminance for a
                    -- strongly-blue source: saturated blue remains bright.
                    local intensity =
                        math.max(
                            r,
                            g,
                ...
                if not maskImage then
                    error(
                        "CreateEditableImage returned nil"
                    )
                end

                maskImage:WritePixelsBuffer(
                    Vector2.zero,
                    Vector2.new(
                        width,
                        height
                ...
                window.Logo.ImageContent =
                    content

                if window.LogoGlow then
                    window.LogoGlow.ImageContent =
                        content
                end

                -- Keep strong refs to runtime image objects.
                window._LogoMaskImage =
                    maskImage

                window._LogoSourceImage =
                    sourceImage

                window._LogoMaskReady =
                    true
            end)

        window._LogoMaskPreparing =
            false

        if not ok then
            -- Permission/API fallback: original asset remains untouched.
            window._LogoMaskError =
                tostring(err)
            ...
            -- Re-apply whichever theme is active after the neutral mask
            -- replaces the original blue pixels.
            local theme =
                Library.Theme
...
function Library:SetAccentColor(accent)
    ...
    local oldTheme = {
        ...
        BlueSelection = COLORS.BlueSelection,

        ToggleOn = TA_AccentExtraColors.ToggleOn,
        IconHolder = TA_AccentExtraColors.IconHolder,
        IconHolderSelected = TA_AccentExtraColors.IconHolderSelected,
        SelectedText = TA_AccentExtraColors.SelectedText,
    }

    local newTheme = TA_ThemeDerived(accent)

    COLORS.Blue = newTheme.Blue
    COLORS.BlueBright = newTheme.BlueBright
    COLORS.BlueDark = newTheme.BlueDark
    COLORS.BlueSelection = newTheme.BlueSelection

    TA_AccentExtraColors.ToggleOn = newTheme.ToggleOn
    TA_AccentExtraColors.IconHolder = newTheme.IconHolder
    TA_AccentExtraColors.IconHolderSelected = newTheme.IconHolderSelected
    TA_AccentExtraColors.SelectedText = newTheme.SelectedText

    Library.Theme.Accent = newTheme.Blue
    Library.Theme.AccentBright = newTheme.BlueBright
    Library.Theme.AccentDark = newTheme.BlueDark
    Library.Theme.AccentSelection = newTheme.BlueSelection
    ...
    for _, window in ipairs(windows) do
        ...
        if window.Logo
            and not window._LogoMaskReady
            and not window._LogoMaskPreparing
            and window.Logo.Image ~= ""
        then
            TA_PrepareTintableLogo(
                window,
                window.Logo.Image
            )
        end

        -- Same asset id, but once the neutral mask is ready this is now a true
        -- full-hue tint instead of multiplying against blue source pixels.
        if window.Logo then
            window.Logo.ImageColor3 =
                newTheme.BlueBright
        end
...
function Library:GetAccentColor()
    return COLORS.Blue
end
...
function Library.new(config)
    ...
    if logoAsset then
        ...
        -- Make the originally-blue logo neutral/tintable while keeping the
        -- exact same rbxassetid.
        TA_PrepareTintableLogo(
            self,
            logoAsset
        )
